...
class GMVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        #kl = ut.kl_normal(m, v, self.z_prior_m.expand(x.size()[0], self.z_dim),
        #                    self.z_prior_v.expand(x.size()[0], self.z_dim))

        nelbo = kl - rec

        rec = rec.sum(-1) / x.size()[0]
        kl = kl.sum(-1) / x.size()[0]
        nelbo = nelbo.sum(-1) / x.size()[0]
    ...
    def negative_iwae_bound(self, x, iw):
        ...
        for i in range(x.size()[0]):
            ...
            exponent = ut.log_bernoulli_with_logits(x_i, x_hat) + \
                    ut.log_normal_mixture(z, prior[0], prior[1]) \
                    - ut.log_normal(z, m, v)
