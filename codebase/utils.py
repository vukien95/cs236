...
def log_normal(x, m, v):
    ...
    # the last dimension
    ################################################################################
    element_wise = - 0.5 * (math.log(2 * np.pi) + v.log() + ((x - m).pow(2) / v))
...
def log_normal_mixture(z, m, v):
    ...
    # in the batch
    ################################################################################
    exponent = -0.5 * torch.log(v).sum(-1) - 0.5 * ((z.unsqueeze(1) - m).pow(2) / v).sum(-1)
    log_prob = log_mean_exp(exponent, -1) - 0.5 * z.size()[-1] * math.log(math.pi * 2)
...
def log_sum_exp(x, dim=0):
    ...
    tmp = max_x + (new_x.exp().sum(dim)).log()
    return tmp
